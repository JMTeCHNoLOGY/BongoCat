...
use crate::core::multiplayer::MultiplayerManager;
...
pub async fn start_gamepad_listing<R: Runtime>(app_handle: AppHandle<R>) -> Result<(), String> {
    ...
    while IS_LISTENING.load(Ordering::SeqCst) {
        while let Some(event) = gilrs.next_event() {
            ...
            app_handle.state::<MultiplayerManager>().publish_input(
                &format!("{:?}", gamepad_event.kind),
                json!({ "name": gamepad_event.name, "value": gamepad_event.value }),
            );
        }
    }
