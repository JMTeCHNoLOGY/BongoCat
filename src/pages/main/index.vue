...
watch(() => modelStore.currentModel, async (model) => {
  ...
  if (multiplayerStore.state === 'disconnected') await handleLoad()
...
watch([() => catStore.window.visible, () => multiplayerStore.state], ([visible, multiplayerState], [, previousState]) => {
  if (multiplayerState === 'disconnected') {
    if (previousState === 'disconnected') {
      if (!visible) return hideWindow()

      showWindow()
      if (!live2d.model) void handleLoad()
    }
    return
  }
...
useTauriListen(LISTEN_KEY.MULTIPLAYER_RENDERER_REQUESTED, () => {
  if (multiplayerStore.state === 'disconnected') return

  handleDestroy()
  hideWindow()
  void emit(LISTEN_KEY.SINGLE_MODEL_RELEASED)
})

useTauriListen(LISTEN_KEY.MULTIPLAYER_MODELS_RELEASED, async () => {
  if (!catStore.window.visible) return

  // A hidden WebView does not render Pixi frames, so show it before waiting
  // for Live2DSprite.ready. The multiplayer window has already released its
  // Cubism models when this acknowledgement is emitted.
  showWindow()
  await handleLoad()
})
